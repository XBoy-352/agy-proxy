...
**Single-file architecture**: `server.mjs` (~557 lines, zero dependencies) + `models.json` for model definitions. No build step, no transpilation, no framework.
...
- **No `models.schema.json`** exists — `models.json` references it but the schema file is absent. Don't create it unless asked.
- **Streaming is faked**: `agy --print` outputs all text at once. The proxy sends SSE chunks as they arrive from stdout, so clients see bursts rather than true token-by-token streaming.
...
- **Model lookup is strict**: if the requested model isn't in `MODEL_MAP`, the request falls back to `DEFAULT_MODEL` silently (no error).
