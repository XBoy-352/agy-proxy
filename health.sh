#!/bin/bash
# Quick health check for agy-proxy
PORT="${AGY_PROXY_PORT:-3457}"
...
if response=$(curl -sf "http://127.0.0.1:${PORT}/health" 2>/dev/null); then
    ...
    echo "✗ HTTP: NOT responding on http://127.0.0.1:${PORT}"
fi

# Check models endpoint
if models=$(curl -sf "http://127.0.0.1:${PORT}/v1/models" 2>/dev/null); then
